...
const Detail = () => {
  ...
  useEffect(() => {
    axios
      ...
      .catch(
        (error) => setRecipeDetail(error.response.data.error)
  ...
  return (
    <div>
      ...
      {!recipeDetail.name && typeof recipeDetail !== "string" ? (
        <Loading />
      ) : (
        <div className="cointainer_details">
          {recipeDetail.name ? (
                ...
                <>
                  {recipeDetail.steps?.map((step, i) => {
                    return (
                      <li key={i}>
                        {step.number
                          ? `step number ${step?.number} - ${step?.step}`
